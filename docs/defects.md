...
| DEF-016 | 2026-08-14 | *(no FR — test infrastructure)* / e2e suite, `profile.spec.ts` — **FEAT-008**'s screen (SCR-WEB-013) | **Open.** A **rotating** single failure in `profile.spec.ts`: two consecutive CI runs of byte-identical code failed on *different* tests, 70/71 passing each time. Run 1 failed UC-007 (`:63`) at `getByLabel("Dark").check()` — "Clicking the checkbox did not change its state", with the log showing the click DID land ("click action done"); run 2 passed UC-007 in 3.6s and failed AC-7 (`:165`) at `expect(afterFirst).toBe(1)`. Both are **races between an assertion and an in-flight save**, not product defects: `.check()` clicks then immediately asserts `checked` on a radio whose state settles only after the preference save resolves; AC-7 captures its intercepted-PATCH count right after the timezone *field renders*, which can precede the PATCH actually being sent, reading `0`. Same genre as DEF-014 in this very file ("a race the machine's speed decides"). **Newly load-bearing:** as of `01c51ff` CI gates deployment (`deploy-staging.yml` runs on a successful CI `workflow_run`), so this flake now blocks releases at random rather than merely annoying. The identical tree passed as PR run `31805330047`; master runs `31806289552` attempts 1 and 2 both failed. **Investigated 2026-09-05, NOT reproduced locally** — four hypotheses tested and **ruled out**: (a) `.check()` losing the race against a slow save — a probe holding the profile PATCH 1.2 s passed, because `.check()` *does* retry through the `disabled={busyField==="theme"}` window; (b) a stale earlier response clobbering a later field via `save()`'s `setTheme(saved.theme)` — a probe delaying only the timezone PATCH kept the theme `dark`; (c) plain machine slowness — CDP `Emulation.setCPUThrottlingRate` at **×6 and ×20** both stayed green; (d) contention — `profile` spec run `--repeat-each=5 --workers=2` gave 25/25 green, and a full-suite run gave 71/71. **Root blocker found instead: the failures produce no evidence.** `playwright.config.ts` set `trace: "on-first-retry"` while `retries` was unset (default **0**), so a trace was *never* written, and `ci.yml` uploaded no artifacts. Fixed both — `trace: "retain-on-failure"` plus a `if: failure()` artifact upload (verified locally: `trace.zip` now written). Deliberately **not** solved with `retries: 1`, which would let the flake pass on retry and exit 0, hiding it while CI gates deployment. **Still open**: the next CI occurrence should finally carry a trace | _open_ | _open_ |
