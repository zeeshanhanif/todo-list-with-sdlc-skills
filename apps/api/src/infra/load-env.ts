 ...
 * CWD set to the workspace directory (`apps/api`), where there is no `.env`, so
 ...
 *
 * - **Resolved by walking up**, not by CWD, so `npm run dev:api` from anywhere
 ...
 *   request — `loadConfig()` is called per-request in a dozen places.)
 *
